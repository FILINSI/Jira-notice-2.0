...
TELEGRAM_CHAT_ID = ''  # ID чата или пользователя. перечислить через запятую

# Добавьте в начало файла после других констант
SUPPORT_STAFF = [
    {"name": "Юдин Константин", "jira_login": "yudin"},
    # Добавьте других сотрудников так же как и тут
]
...
class DutySystem:
    def __init__(self):
        today = datetime.now().date()  # Используем .date() для согласованности
        ...
        self.duty_users = {
            "yudin": {
                ...
                "username": "@yudin",
                "schedule_type": "2/2",
                "start_date": today  # Используем date вместо datetime
            },
            "obordoev": {
                "name": "Обордоев Вадим",
                "username": "@obordoev",
        ...
        if action == 'take':
            ...
            try:
                ...
                if transition_id:
                    ...
                    await query.answer("Невозможно выполнить переход в статус 'Анализ обращения'")
            except Exception as e:
...
async def authenticate_and_notify():
    ...
    while retry_count < max_retries:
        try:
            ...
            await send_telegram_message("✅ Бот успешно подключн к JIRA\n🔍 Отслеживаю новые задачи...\n\n🔄 Обновление: поправлен баг с получением двух сообщений о новой задаче и назначением исполнителя в jira")
            return jira
...
async def check_new_issues(jira):
    ...
    try:
        ...
        jql_query = 'project = "Поддержка Informatics" AND status in ("Новое обращение", Ожидание, "ТИКЕТ СОЗДАН")'
